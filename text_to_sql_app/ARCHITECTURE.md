...
4. **_coerce_numeric_comparisons**  
   - Wrap numeric comparisons (`>`, `<`, `>=`, `<=`, `=`) with `TRY_CAST(column AS DOUBLE)`.

5. **_coerce_numeric_operations**  
   - Cast all arithmetic (`-`, `+`, `*`, `/`) operands to `DOUBLE`.

6. **_cast_numeric_aggregates**  
   - Additional safety for aggregates (e.g., `SUM(TRY_CAST(...))` if needed).

7. **_validate_sql_columns**  
