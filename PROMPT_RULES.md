...
- Rules such as "Follow join specifications exactly," "Group by all non-aggregated columns," "Return only the final SQL query without explanations," "Do NOT use TO_DATE/TO_CHAR—prefer TRY_CAST(column AS DATE) or CAST(column AS VARCHAR)," and "When joining another table, aggregate it first in a CTE grouped by the join keys before joining."
...
4. **Rewrite unsupported functions** – converts `TO_DATE(expr, pattern)` → `TRY_CAST(expr AS DATE)` and `TO_CHAR(expr, pattern)` → `CAST(expr AS VARCHAR)`.
5. **Cast numeric comparisons** – all comparisons with numeric literals (quoted or unquoted) automatically wrap the column in `TRY_CAST(... AS DOUBLE)`.
6. **Cast arithmetic operations** – subtraction, addition, multiplication, and division between columns wrap both operands in `TRY_CAST(... AS DOUBLE)`.
7. **Detect unsafe aggregations with joined dimensions** – if a query aggregates the primary table while joining another table without pre-aggregating that dimension, the sanitiser raises a `ValueError` instructing the model to build a summary CTE grouped by the join keys (e.g., `WITH dim_summary AS (...)`).
8. **Validate schema** – confirm no missing tables/columns remain.
9. **Execute** – only if all validations pass; otherwise, the correction loop restarts with the error message and sanitized SQL.
